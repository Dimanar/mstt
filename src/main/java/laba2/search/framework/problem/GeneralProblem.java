package laba2.search.framework.problem;
...
public class GeneralProblem<S, A> implements Problem<S, A> {
	 ...
	 * @param actionsFn a description of the possible actions available to the agent.
	 * @param resultFn a description of what each action does; the formal name for
	 * this is the transition model, specified by a function
	 * RESULT(s, a) that returns the state that results from doing
	 * action a in state s.
	 * @param goalTest test determines whether a given state is a goal state.
	 * @param stepCostFn a path cost function that assigns a numeric cost to each path.
	 ...
	 *
	 * @param initialState the initial state that the agent starts in.
	 * @param actionsFn a description of the possible actions available to the agent.
	 * @param resultFn a description of what each action does; the formal name for
	 * this is the transition model, specified by a function
	 * RESULT(s, a) that returns the state that results from doing
