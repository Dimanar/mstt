package laba2.probability.hmm.impl;
...
import laba2.probability.hmm.HiddenMarkovModel;
import laba2.probability.proposition.AssignmentProposition;
import laba2.probability.util.ProbabilityTable;
import laba2.util.Util;
import laba2.util.math.Matrix;
...
public class HMM implements HiddenMarkovModel {
	 ...
	 *
	 * @param stateVariable the single discrete random variable used to describe the
	 * process states 1,...,S.
	 * @param transitionModel the transition model:<br>
	 ...
	 * = i).
	 * @param sensorModel the sensor model in matrix form:<br>
	 * P(e<sub>t</sub> | X<sub>t</sub> = i) for each state i. For
	 * mathematical convenience we place each of these values into an
	 * S * S diagonal matrix.
	 * @param prior the prior distribution represented as a column vector in
	 * Matrix form.
	 */
