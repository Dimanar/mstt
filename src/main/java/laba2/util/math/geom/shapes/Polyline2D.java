...
public final class Polyline2D implements IGeometric2D {
	...
	public double rayCast(Ray2D ray) {
		...
		for(int i = 0; i < edges.length; i++) {
			if(!ray.getDirection().isParallel(edges[i])) {
				...
				final double len1 =
						(vertexes[i].getY() * edges[i].getX() - ray.getStart().getY() * edges[i].getX() - vertexes[i].getX() * edges[i].getY() + ray.getStart().getX() * edges[i].getY()) / divisor;
				if(len1 > 0) {
					final double len2 =
							(ray.getDirection().getY() * ray.getStart().getX() - ray.getDirection().getY() * vertexes[i].getX() - ray.getDirection().getX() * ray.getStart().getY() + ray.getDirection().getX() * vertexes[i].getY()) / divisor;
	...
	public Polyline2D transform(TransformMatrix2D matrix) {
		...
		return new Polyline2D(vertexesNew,
				isClosed || (Util.compareDoubles(vertexesNew[0].getX(), vertexesNew[vertexes.length - 1].getX()) && Util.compareDoubles(vertexesNew[0].getY(),
						vertexesNew[vertexes.length - 1].getY())));
	}
}
