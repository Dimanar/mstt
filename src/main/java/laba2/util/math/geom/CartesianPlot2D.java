...
import laba2.util.math.geom.shapes.Ray2D;
...
public class CartesianPlot2D {
	 ...
	 * this function may return {@code Double.POSITIVE_INFINITY} for a ray that intersects with an obstacle that is further away than the rayRange from the
	...
	public double rayCast(Ray2D ray) {
		...
		Rect2D rayBounding = new Rect2D(ray.getStart().getX() - rayRange, ray.getStart().getY() - rayRange, ray.getStart().getX() + rayRange,
				ray.getStart().getY() + rayRange);
