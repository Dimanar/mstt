package laba2.logic.propositional.inference;
...
import laba2.logic.propositional.kb.KnowledgeBase;
import laba2.logic.propositional.kb.data.Clause;
import laba2.logic.propositional.kb.data.Literal;
import laba2.logic.propositional.kb.data.Model;
import laba2.logic.propositional.parsing.ast.ComplexSentence;
import laba2.logic.propositional.parsing.ast.Connective;
import laba2.logic.propositional.parsing.ast.PropositionSymbol;
import laba2.logic.propositional.parsing.ast.Sentence;
import laba2.logic.propositional.transformations.ConvertToConjunctionOfClauses;
import laba2.logic.propositional.transformations.SymbolCollector;
import laba2.util.Tasks;
import laba2.util.Util;
import laba2.util.datastructure.Pair;
 ...
 */
public class DPLL implements SatisfiabilityChecker, EntailmentChecker {
	...
	public boolean isSatisfiable(Sentence s) {
		...
		Set<Clause> clauses = ConvertToConjunctionOfClauses.apply(s)
				.getClauses();
	...
	public boolean isEntailed(KnowledgeBase kb, Sentence alpha) {
		...
		kbAndNotAlpha.addAll(ConvertToConjunctionOfClauses.apply(notQuery).getClauses());
	 ...
	 * @param symbols a list of unassigned symbols.
	 * @param model contains the values for assigned symbols.
	 ...
	 *
	 * @param symbols a list of currently unassigned symbols in the model (to be
	...
	protected Pair<PropositionSymbol, Boolean> findUnitClause(
		...
		for(Clause c : clauses) {
			...
			if(unassigned != null) {
				result = new Pair<>(unassigned.getAtomicSentence(), unassigned.isPositiveLiteral());
				break;
			}
		}
	...
	protected List<PropositionSymbol> minus(List<PropositionSymbol> symbols,
	                                        PropositionSymbol p) {
		return symbols.stream().filter(sym -> !p.equals(sym)).collect(Collectors.toList());
	}
}